...
void INPUT_COLLECT(void)
{
    ...
    if((!S1)&&(!S2)&&(S3)&&(S5)&&(!S6)&&(S7)&&(!FLAG_QS_MOTOR_IN)&&(!FLAG_QS_MOTOR_OUT)&&(!FLAG_ALARM)&&(!FLAG_ALARM_CMD)&&(!FLAG_ALARM_CMD1))
    {
        time_2ms_1++;
        if(time_2ms_1>10)
        {
...
//    }
    ...
    if(((!S1)&&(S2)&&(!S3)&&(S6)&&(!FLAG_QS_MOTOR_IN)&&(!FLAG_QS_MOTOR_OUT)&&(!FLAG_ALARM)&&(!FLAG_ALARM_CMD)&&(!FLAG_ALARM_CMD1)))    //在中间位可分闸，保护分闸指令也可分闸，故不判断合闸位信号，保护分闸不判断复位信号
    {
...
//           FLAG_RESET_ALARM=0;           
    ...
    {
        RELAY_LOCK_LAT=0;
        if((S1&&(FLAG_QS_MOTOR_IN))||(S1&&(FLAG_QS_MOTOR_OUT)))      //电机运行过程中联锁信号断开
        {
            time_2ms_7++;
            if(time_2ms_7>10)
            {
