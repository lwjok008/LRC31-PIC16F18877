...
uchar out_order=1,out_stop_order=1,in_order=1,in_stop_order=1,ES_in_stop_order=1,ES_in_order=1,ES_out_stop_order=1,ES_out_order=1;
uint I_current=0;
uint led_time_40ms;
void ALARM_CTRL(void)
{
    ...
    else
    {
        ...
        else if((!FLAG_ALARM_CMD)&&(!FLAG_ALARM_CMD1)&&(!FLAG_ALARM_CMD2)&&(!FLAG_ALARM_RUN)&&(!FLAG_OVER_TIME_30S)&&(!FLAG_OVER_CURRENT)&&(!FLAG_ALARM_OVER1)&&(!FLAG_ALARM_OVER2))    //故障全部恢复，故障常闭继电器带电断开
        {
            RELAY_ALARM_LAT=1;
        }
    }
...
//    if(FLAG_ALARM)
    {
        ...
        else if((FLAG_ALARM_CMD) || (FLAG_ALARM_CMD1)|| (FLAG_ALARM_CMD2)||(FLAG_ALARM_RUN)) //指令异常和联锁、主母线异常快闪
        {
            led_time_40ms++;
            if(led_time_40ms>=3)
            {
